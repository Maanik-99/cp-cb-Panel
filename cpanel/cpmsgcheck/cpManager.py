#!/usr/bin/env python3
"""
CPanel Manager – Create/Delete email accounts, Delete forwarders, Check messages (IMAP)
"""
...
# =========================================================
# OPTION 3: DELETE ALL FORWARDERS (all panels)
# =========================================================
def delete_forwarders_all_panels():
    clear_console()
    print("=" * 70)
    print("        DELETE ALL FORWARDERS – ALL PANELS")
    print("=" * 70)
    ...
    for idx, (host, user, domain, token) in enumerate(configs):
        ...
        print(f"  [*] Found {len(forwarders)} forwarders, deleting...")
        deleted = 0
        failed = 0
        skipped = 0
...
# =========================================================
# OPTION 4: CHECK MESSAGES (IMAP)
# =========================================================
def check_messages_all_panels():
    clear_console()
    print("=" * 70)
    print("        CHECK MESSAGES (IMAP) – ALL PANELS")
    print("=" * 70)
    ...
    for pidx, (host, user, domain, _) in enumerate(configs):
        ...
        for email_addr in emails:
            ...
            if not password:
                print(f"  ⚠ No password for {email_addr}")
                continue
            ...
            if subjects and not subjects[0].startswith("IMAP ERROR"):
                ...
                for subj_idx, subj in enumerate(subjects, 1):
                    report_lines.append(f"{subj_idx}. {subj}")
...
def main():
    while True:
        ...
        print("3. Delete ALL forwarders (all panels)")
        print("4. Check messages (IMAP) using passwords.txt")
        print("5. Exit")
        print("-" * 70)
